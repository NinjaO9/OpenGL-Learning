...
void main()
{
	...
	result *= vec3(texture(material.diffuseMap, texCoord).xyz);
	result += vec3(texture(material.emission, texCoord).xyz);
...
vec3 calculateDirectionalLight(DirectionalLight light)
{
	...
	vec3 ambient = light.ambient * vec3(texture(material.diffuseMap, texCoord)) * light.color;
...
vec3 calculateSpotLight(SpotLight light)
{
	...
	vec3 diffuse = light.diffuse * getDiffuseLighting(norm, lightDir, light.color) * intensity;

	vec3 specular = light.specular * getSpecularLighting(norm, lightDir, light.color) * intensity;
...
vec3 getDiffuseLighting(vec3 norm, vec3 dir, vec3 lColor)
{
	...
	vec3 diffuse = (diff * vec3(texture(material.diffuseMap, texCoord))) * lColor;
	return diffuse;
}
